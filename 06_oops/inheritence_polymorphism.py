...
class Dog(Animal):
    def __init__(self, name: str, age: int, num_legs: int, breed: str) -> None:
        # Take the common fetures and pass to the parent(super) class
    ...
    def sniff(self, item: str) -> None:
        print(f"{self.name} sniffing out {item}")
...
d1 = Dog("Whisky", age=5, num_legs=4, breed="Dobberman")
print(d1)
d1.talk()
d1.sniff("ball")
...
class Dino(Animal):
    ...
    def talk(self) -> None:
        print(f"{self.name} says urgghhh!")

    def hunt(self) -> None:
        print(f"{self.name} is out for hunting....")


dino1 = Dino("Adam", age=8, num_legs=4, breed="T- rex")
